...
# --- 1. التهيئة والاتصال بقاعدة البيانات السحابية Supabase ---
@st.cache_resource
...
# --- 3. جلب بيانات الطقس وتوليد ملفات الملاحة والـ QR ---
...
# --- 5. شاشة المصادقة والدخول السحابية ---
            ...
            if supabase:
                ...
                if res.data and check_password(password, res.data[0]["password_hash"]):
                    if input_pin == "2087":
                        ...
                        st.success("تم الدخول والتشفير السحابي بنجاح!")
                        st.rerun()
                    else:
                        st.error("⚠️ رقم التشفير (PIN) خاطئ.")
                else:
                    st.error("⚠️ بيانات الدخول غير صحيحة.")
            else:
                st.warning("⚠️ يرجى ضبط إعدادات Supabase Secrets.")

    with tab_signup:
        new_email = st.text_input("البريد الإلكتروني الجديد", key="s_email")
        ...
        if st.button("إنشاء الحساب"):
            if new_email and new_pass:
                ...
                if supabase:
                    try:
                        supabase.table("users").insert({"email": new_email.strip().lower(), "password_hash": hashed}).execute()
                        st.success("تم تسجيل الحساب! يمكنك الآن تسجيل الدخول.")
                    except Exception:
...
# --- 6. الواجهة الرئيسية V5.1 Ultimate ---
    ...
    # الميزة الجديدة 1: محاكاة الطقس المستقبلي
    ...
    for wpt, color in zip([target_10, target_9, target_8], ["#00ff66", "#ffaa00", "#ffff00"]):
        folium.Rectangle(
            ...
            popup=f"<b>{wpt['name']}</b><br>{wpt['desc']}"
        ).add_to(m)
